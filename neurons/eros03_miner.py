"""Poker44 miner — 147-feature behavioral tree ensemble (eros03 deployment).
...
v1.0-s7 artifact (its own trained weights; see miner_model/ for the pipeline).
"""
...
ARTIFACT = os.getenv("POKER44_ARTIFACT", str(REPO_ROOT / "artifacts" / "poker44_model_s7.joblib"))
MODEL_NAME = "poker44-behavioral-trees-b"
MODEL_VERSION = "v1.0-s7"
REPO_URL = "https://github.com/rockmania99/poker44-wmean"
FALLBACK_SCORE = 0.45
...
class Miner(BaseMinerNeuron):
    ...
    async def forward(self, synapse: DetectionSynapse) -> DetectionSynapse:
        ...
        if capture_chunks is not None:
            capture_chunks(chunks, tag="eros03", base_dir=str(REPO_ROOT))
        t0 = time.time()
        try:
